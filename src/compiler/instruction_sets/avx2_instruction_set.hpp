...
private:
    ...
    void emitVectorizedMathCall1Arg(asmjit::x86::Assembler& a, int dstReg, int srcReg,
        ...
        // CRITICAL: On Windows, we must allocate shadow space (32 bytes) BEFORE computing
        // any pointers, otherwise the LEA instructions will compute addresses relative to
        // the pre-shadow RSP, but the function will be called with post-shadow RSP.
        //
        ...
        // Set up function arguments - pointers are now correct relative to final RSP
#ifdef _WIN32
