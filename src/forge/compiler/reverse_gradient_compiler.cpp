...
// Helper function to convert ComputationGraph to GraphView (equivalent to TapePresso's toForgeGraphView)
inline forge::x86::IInstructionSet::GraphView toGraphView(const forge::core::ComputationGraph& graph) {
    forge::x86::IInstructionSet::GraphView view;
    view.nodes = reinterpret_cast<const forge::x86::IInstructionSet::GraphView::Node*>(graph.nodes.data());
    view.constPool = graph.constPool.data();
    view.nodeCount = graph.nodes.size();
    return view;
}
#include <stdexcept>
#include <iostream>
...
void ReverseGradientCompiler::generateGradientOperation(
    ...
    const forge::x86::CompilerConfig* config) {
    ...
    switch(node.op) {
        ...
        case OpCode::Mul:
            ...
            if (node.a < graph.nodes.size() && graph.nodes[node.a].needsGradient) {
                instructionSet->emitLoadValueForGradient(a, 1, node.b, toGraphView(graph), &constantMap, constPoolLabel);
                instructionSet->emitMul(a, 1, 0);  // xmm1 = grad[nodeId] * value[b]
                instructionSet->emitAccumulateGradient(a, 1, node.a);
            }
            
            if (node.b < graph.nodes.size() && graph.nodes[node.b].needsGradient) {
                instructionSet->emitLoadValueForGradient(a, 1, node.a, toGraphView(graph), &constantMap, constPoolLabel);
        ...
        case OpCode::Div:
        {
            ...
            // Gradient for b
            if (node.b < graph.nodes.size() && graph.nodes[node.b].needsGradient) {
                instructionSet->emitLoadValueForGradient(a, 2, node.a, toGraphView(graph), &constantMap, constPoolLabel);  // Load value[a] (numerator)
        ...
        case OpCode::Sqrt:
            ...
            if (node.a < graph.nodes.size() && graph.nodes[node.a].needsGradient) {
                instructionSet->emitLoadGradient(a, 0, nodeId);
                instructionSet->emitLoadValueForGradient(a, 1, nodeId, toGraphView(graph), &constantMap, constPoolLabel);  // Load sqrt result
        ...
        case OpCode::Exp:
            ...
            if (node.a < graph.nodes.size() && graph.nodes[node.a].needsGradient) {
                instructionSet->emitLoadGradient(a, 0, nodeId);
                instructionSet->emitLoadValueForGradient(a, 1, nodeId, toGraphView(graph), &constantMap, constPoolLabel);  // exp(x) result
        ...
        case OpCode::Pow:
        {
            ...
            instructionSet->emitLoadValueForGradient(a, 1, node.a, toGraphView(graph), &constantMap, constPoolLabel);  // x in reg 1
            instructionSet->emitLoadValueForGradient(a, 2, node.b, toGraphView(graph), &constantMap, constPoolLabel);  // y in reg 2
            ...
            if (node.a < graph.nodes.size() && graph.nodes[node.a].needsGradient) {
                ...
                instructionSet->emitLoadGradient(a, 0, nodeId);
                instructionSet->emitLoadValueForGradient(a, 2, node.b, toGraphView(graph), &constantMap, constPoolLabel);  // Reload y
            ...
            if (node.b < graph.nodes.size() && graph.nodes[node.b].needsGradient) {
                ...
                instructionSet->emitLoadGradient(a, 0, nodeId);
                instructionSet->emitLoadValueForGradient(a, 7, nodeId, toGraphView(graph), &constantMap, constPoolLabel);  // x^y (result) in reg 7
        ...
        case OpCode::IntIf:
        {
            ...
            instructionSet->emitLoadValueForGradient(a, 0, node.a, toGraphView(graph), &constantMap, constPoolLabel);  // 0.0 or 1.0
        ...
        case OpCode::Min:
            ...
            {
                instructionSet->emitLoadValueForGradient(a, 0, node.a, toGraphView(graph), &constantMap, constPoolLabel);
                instructionSet->emitLoadValueForGradient(a, 1, node.b, toGraphView(graph), &constantMap, constPoolLabel);
...
void ReverseGradientCompiler::stitchGradientPass(
    asmjit::x86::Assembler& a,
    const forge::core::ComputationGraph& graph,
    const std::unordered_map<forge::core::NodeId, generators::ConstantInfo>& constantMap,
    const asmjit::Label& constPoolLabel,
    ...
    for (forge::core::NodeId outputNode : graph.outputs) {
    ...
    for (int64_t i = graph.nodes.size() - 1; i >= 0; --i) {
        ...
        forge::core::NodeId nodeId = static_cast<forge::core::NodeId>(i);
