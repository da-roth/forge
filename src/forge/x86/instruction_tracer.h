...
// Helper class for emitting safe runtime tracing in instruction sets
// This provides the "duplicate -> store -> call tracer" pattern
...
public:
    ...
    // Emit tracing for AVX2 (YMM) registers
    void emitTraceYMM(asmjit::x86::Assembler& a, asmjit::x86::Ymm liveReg, 
            ...
            if (!config.enableSmartTraceFilter) {
                ...
                if (instructionCounter == 0) {
                    std::cout << "[Compiling] Trace points: ";
                }
    ...
    // Emit tracing for SSE2 (XMM) registers
    ...
    // Reset instruction counter (useful for new function compilation)
};
