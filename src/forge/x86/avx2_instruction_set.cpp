#include "avx2_instruction_set.h"
#include "ymm_register_allocator.h"  // Use YMM-specific allocator
#include "runtime_trace.h"           // For runtime tracing
...
extern "C" double call_std_sin(double x) {
    // CRITICAL: Disable FMA3 for all transcendental functions in AVX/JIT context
    static bool fma3_disabled = false;
    if (!fma3_disabled) {
        _set_FMA3_enable(0);
        fma3_disabled = true;
...
// Memory operations
void AVX2InstructionSet::emitLoad(asmjit::x86::Assembler& a, int dstReg, uint32_t nodeId) {
...
void AVX2InstructionSet::emitStore(asmjit::x86::Assembler& a, int srcReg, uint32_t nodeId) {
...
void AVX2InstructionSet::emitOptimizedLoad(asmjit::x86::Assembler& a, int dstReg, uint32_t nodeId) {
    // Use alignment-aware load
    emitLoad(a, dstReg, nodeId);
}

void AVX2InstructionSet::emitOptimizedStore(asmjit::x86::Assembler& a, int srcReg, uint32_t nodeId) {
    // Use alignment-aware store
    emitStore(a, srcReg, nodeId);
}

// Gradient operations
void AVX2InstructionSet::emitLoadGradient(asmjit::x86::Assembler& a, int dstReg, uint32_t nodeId) {
...
void AVX2InstructionSet::emitStoreGradient(asmjit::x86::Assembler& a, int srcReg, uint32_t nodeId) {
    // Store 4 gradient values to workspace
    size_t offset = nodeId * 4 * sizeof(double);  // 4 doubles per node
    a.vmovupd(asmjit::x86::ymmword_ptr(asmjit::x86::rsi, offset), getYmmRegister(srcReg));
}

void AVX2InstructionSet::emitAccumulateGradient(asmjit::x86::Assembler& a, int srcReg, uint32_t nodeId, int tempReg) {
...
void AVX2InstructionSet::emitLoadValueForGradient(asmjit::x86::Assembler& a, int dstReg, uint32_t nodeId,
    ...
    const auto* constMap = static_cast<const std::unordered_map<uint32_t, AVX2ConstantInfo>*>(constantMap);
